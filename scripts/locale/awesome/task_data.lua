...
function displayTasks(tasks_sorting)
	...
	if tasks_sorting == "cpu" then
		execute_command = "ps -o pcpu,pmem,size,pid,user,comm | head -1; ps -Aeo pcpu,pmem,size,pid,user,comm | sort -k 1 -gr | head -10"
	elseif (tasks_sorting == "mem") then
		execute_command = "ps -o pmem,size,pcpu,pid,user,comm | head -1; ps -Aeo pmem,size,pcpu,pid,user,comm | sort -k 1 -gr | head -10"
	end
